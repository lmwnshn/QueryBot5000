...
class Preprocessor:
    ...
    def get_params(self, query):
        ...
        return params.droplevel(0).squeeze()
    ...
    def substitute_params(query_template, params):
        def substitute(param_idx):
            query = query_template
            param_vec = params[param_idx]
            keys = [f'${i}' for i in range(1, len(param_vec) + 1)]
            for k, v in zip(keys, param_vec):
                query = query.replace(k, v)
            return query

        return [substitute(i) for i in range(len(params))]
